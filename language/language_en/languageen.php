<?php

define("NAME_TEXT","I'am Helmi");

define("PASSIONNED_TEXT","And, above all, passionate about what I do!");

define("INTERET_TEXT","An interest in design and innovative interfaces and a great deal of imagination");

define("CONTACT_TEXT","Let's chat !");

define("PROJECT_TEXT","Several finished projects are in progress but first of all let's start");


define("ONLINE_TEXT", "online");

define("CALL_TEXT", "Missed voice call to");

define("CALL_APP_TEXT", "WhatsApp voice call");

define("RESPOND_TEXT", "Swipe up <br> to accept");

define("CONCEV_TEXT", 'let\'s design');

define("UNIQUE_TEXT", " a unique experience,");

define("ELEGANTE_TEXT", " elegant and at the same time");

define("ACCROCHE_TEXT", " catchy !");

define("STRATEGY_TEXT", "An SEO strategy");

define("ACTION_TEXT", " and an action plan to optimize");

define("REFER_TEXT", " natural referencing.");

define("IMAGINE_TEXT", "Imagine");

define("ENSEMBLE_TEXT", " a set of bricks, ");

define("EMBOITE_TEXT", " that overlap each other ");

define("FORME_TEXT", " to form ");

define("YOU_TEXT", "your");

define("WEBSITE_TEXT", " website ! ");

define("ESTHETIQUE_TEXT", "A clear aesthetic that is both ergonomic <br> and responsive that will fit on all your <br> devices...");

define("DISCOVER_TEXT", "Discover");

define("MY_PROJECT_TEXT", "my projects");

define("COMPETENCE_TEXT", "My skills");

define("PROJECT_TITLE", "Web project management");

define("PROJECT_TITLE_TEXT", "Showcase site, events, e-commerce, redesign, single page application");

define("CONCEPT_TITLE", "Graphic design");

define("CONCEPT_TITLE_TEXT", "Modeling, user stories, wireframes, web interfaces, visual accessibility");

define("DEV_TITLE", "Development of cross-platform applications");

define("DEV_TITLE_TEXT", "Compatible with all media, tablet and mobile application");

define("DEP_TITLE", "Web application deployment");

define("DEP_TITLE_TEXT", "Domain name and hosting");

define("ABOUT_TITLE", "Who am I?");

define("KNOW_TEXT", "To know me better - ");

define("RESPOND_FAST_TEXT", "I will answer you as soon as possible");

define("SEND_BUTTON", "Send");

define("NAME", "Name");

define("FIRSTNAME_TEXT", "Firstname");

define("PHONE_TEXT", "Phone");

define("MESSAGE_TEXT", "Your message");

define("PROJECT", "Project");

define("ABOUT", "About");

define("COMPETENCE", "Skills");

define("DISCOVER", "Discover...");

define("FRENCH", "French");

define("ENGLISH", "English");